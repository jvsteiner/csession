...
- Produces:
  ...
  - `replaceAllText(lines: string[], from: string, to: string): { lines: string[]; replaced: number }` — added in Task 10's fix round: an UNANCHORED replace for the session-id swap. `rewritePrefix` anchors on a path boundary so `/Code/app` cannot rewrite `/Code/app2`; that rule is wrong for a UUID and breaks `<id>.jsonl`.
