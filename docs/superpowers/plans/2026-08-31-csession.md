...
const FIXTURE = [
  ...
  JSON.stringify({ type: "assistant", cwd: "/Users/jamie/Code/app", version: "2.1.246", sessionId: "s1",
    message: { content: [{ type: "text", text: "see /Users/jamie/Code/app/src/a.ts and /Users/jamie/Documents/n.md" }] } }),
].join("\n") + "\n";
...
test("parseManifest rejects a newer schema with exit code 3", () => {
  const err = assert.throws(() => parseManifest(JSON.stringify({ ...GOOD, schema: 999 }))) as CorruptBundleError;
  assert.equal(err.exitCode, 3);
  assert.match(err.message, /schema/i);
});

test("parseManifest rejects unparseable JSON", () => {
  const err = assert.throws(() => parseManifest("{not json")) as CorruptBundleError;
  assert.equal(err.exitCode, 3);
});
...
test("parseManifest rejects a missing required field", () => {
  ...
  const err = assert.throws(() => parseManifest(JSON.stringify(bad))) as CorruptBundleError;
  assert.equal(err.exitCode, 3);
...
```bash
cd /Users/jamie/Code/csession
git add src/errors.ts src/manifest.ts src/manifest.test.ts
...
test("a bundle missing manifest.json is corrupt", () => {
  ...
  try {
    ...
    const err = assert.throws(() => readBundle(out)) as CorruptBundleError;
    assert.equal(err.exitCode, 3);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
...
function scaffold(): { root: string; sessionId: string; cleanup: () => void } {
  const root = mkdtempSync(join(tmpdir(), "csession-exp-"));
...
test("the manifest sha256 matches the shipped transcript, after redaction", () => {
  ...
  try {
    ...
    assert.equal(m.session.sha256, createHash("sha256").update(files["session.jsonl"]!, "utf8").digest("hex"));
  } finally {
    rmSync(out, { force: true });
    s.cleanup();
  }
...
test("an unknown session id is a user error, exit code 1", () => {
  const s = scaffold();
  try {
    const err = assert.throws(() => exportCommand(["nope"], { out: "/tmp/x.ccsession" }, s.root, NOW)) as { exitCode: number };
...
test("omitting -o is a user error", () => {
  const s = scaffold();
  try {
    const err = assert.throws(() => exportCommand([s.sessionId], {}, s.root, NOW)) as { exitCode: number };
    assert.equal(err.exitCode, 1);
  } finally {
    s.cleanup();
  }
...
test("inspect with no file is a user error", () => {
  const err = assert.throws(() => inspectCommand([])) as { exitCode: number };
  assert.equal(err.exitCode, 1);
});
```
...
test("a commit mismatch refuses with exit 2 and writes nothing", () => {
  ...
  try {
    ...
    const err = assert.throws(() => importCommand([bundle], { root: r.root }, r.root)) as { exitCode: number; message: string };
...
test("a remote mismatch refuses with exit 2", () => {
  ...
  try {
    ...
    const err = assert.throws(() => importCommand([bundle], { root: r.root }, r.root)) as { exitCode: number };
    assert.equal(err.exitCode, 2);
  } finally {
    rmSync(dir, { recursive: true, force: true });
    r.cleanup();
...
test("a corrupted transcript fails the sha check with exit 3 and writes nothing", () => {
  ...
  try {
    ...
    const err = assert.throws(() => importCommand([out], { root: r.root }, r.root)) as { exitCode: number };
    assert.equal(err.exitCode, 3);
    assert.equal(existsSync(sessionFilePath(r.root, SESSION_ID)), false);
  } finally {
    rmSync(dir, { recursive: true, force: true });
...
test("no --root and no matching checkout is a user error", () => {
  ...
  try {
    ...
    const err = assert.throws(() => importCommand([bundle], {}, elsewhere)) as { exitCode: number };
    assert.equal(err.exitCode, 1);
  } finally {
    rmSync(dir, { recursive: true, force: true });
